...
if (!$MMRPG_CONFIG['CACHE_INDEXES'] || !file_exists($players_cache_path)){
  ...
  while (false !== ($filename = readdir($data_players))) {
    ...
    if (preg_match('#^player\.[-_a-z0-9]+\.php$#i', $filename)){
      ...
      $this_player_token = preg_replace('#^player\.([-_a-z0-9]+)\.php$#i', '$1', $filename);
      ...
      $this_player_markup[1] = preg_replace('#\$player = array\(#i', "\$mmrpg_index['players']['{$this_player_token}'] = array(", $this_player_markup[1]);
