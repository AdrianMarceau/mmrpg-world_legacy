...
if (!$MMRPG_CONFIG['CACHE_INDEXES'] || !file_exists($fields_cache_path)){
  ...
  while (false !== ($filename = readdir($data_fields))) {
    ...
    if (preg_match('#^field\.[-_a-z0-9]+\.php$#i', $filename)){
      ...
      $this_field_token = preg_replace('#^field\.([-_a-z0-9]+)\.php$#i', '$1', $filename);
      ...
      $this_field_markup[1] = preg_replace('#\$field = array\(#i', "\$mmrpg_index['fields']['{$this_field_token}'] = array(", $this_field_markup[1]);
