...
$types_cache_path = $MMRPG_CONFIG['ROOTDIR'].'data/cache/'.'cache.types.'.$MMRPG_CONFIG['CACHE_DATE'].'.php';
...
if (!$MMRPG_CONFIG['CACHE_INDEXES'] || !file_exists($types_cache_path)){
  ...
  while (false !== ($filename = readdir($data_types))) {
    ...
    if (preg_match('#^type\.[-_a-z0-9]+\.php$#i', $filename)){
      ...
      $this_type_token = preg_replace('#^type\.([-_a-z0-9]+)\.php$#i', '$1', $filename);
      ...
      $this_type_markup[1] = preg_replace('#\$type = array\(#i', "\$mmrpg_index['types']['{$this_type_token}'] = array(", $this_type_markup[1]);
