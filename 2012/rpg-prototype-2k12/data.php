...
// Check and define the WAP flag
$this_battle->flags['wap'] = $flag_wap ? true : false;

//die('wtf<br /><pre>$this_errors : '.print_r($this_field, true).'</pre>');
...
elseif ($this_action == 'start'){
  ...
  $event_body .= "{$this_battle->battle_field->print_field_name()}<br />";
  $event_options = array();
...
// Else if the player's robot using an ability or scan
elseif ($this_action == 'ability' || $this_action == 'scan'){
...
if ($this_battle->battle_status != 'complete'){
  ...
  ob_start();
    ...
    if ($this_robot->robot_energy > 0){
      ...
      ?><a class="button action_scan" type="button" data-action="scan">Scan</a><?
      ?><a class="button action_option" type="button" data-panel="option">Option</a><?
  ...
  ob_start();
    ...
    $temp_options[] = '<a class="button action_option block_1" type="button" data-action="prototype">Prototype Menu</a>';
    // Display an option for restarting this battle from scratch
    $temp_options[] = '<a class="button action_option block_2" type="button" data-action="restart">Restart Battle</a>';
    ...
    if (!empty($this_robot->robot_abilities)){
      ...
      foreach ($this_robot->robot_abilities AS $ability_key => $ability_token){
        ...
        if (isset($mmrpg_index['abilities'][$ability_token])){
          ...
          ?><a class="button action_ability ability_<?= $temp_ability->ability_token ?> type_<?= $temp_ability->ability_type ?> block_<?= $ability_key + 1 ?>" type="button" data-action="ability_<?= $temp_ability->ability_id.'_'.$temp_ability->ability_token ?>" title="<?= $temp_ability_details ?>"><?= $temp_ability->ability_name ?></a><?
        }
      }
      // If there were less than 8 abilities, fill in the empty spaces
      if ($num_abilities < 8){
        for ($i = $num_abilities; $i < 8; $i++){
          // Display an empty button placeholder
    ...
    if (!empty($this_player->player_robots)){
      ...
      foreach ($this_player->player_robots AS $robot_key => $switch_robotinfo){
        ...
        if (isset($mmrpg_index['robots'][$switch_robotinfo['robot_token']])){
          ...
          ?><a title="<?=$temp_robot_title?>" class="button <?= !$allow_button ? 'button_disabled' : '' ?> action_switch switch_<?= $temp_robot->robot_token ?> status_<?= $temp_robot->robot_status ?> block_<?= $robot_key + 1 ?>" type="button" <?if($allow_button):?>data-action="switch_<?= $temp_robot->robot_id.'_'.$temp_robot->robot_token ?>"<?endif;?>><?= $temp_robot->robot_name ?></a><?
        }
      }
    ...
    if (!empty($target_player->player_robots)){
      ...
      foreach ($target_player->player_robots AS $robot_key => $scan_robotinfo){
        ...
        if (isset($mmrpg_index['robots'][$switch_robotinfo['robot_token']])){
          ...
          // Now use the new object to generate a snapshot of this switch button
          ?><a title="<?=$temp_robot_title?>" class="button <?= !$allow_button ? 'button_disabled' : '' ?> action_scan scan_<?= $temp_robot->robot_token ?> status_<?= $temp_robot->robot_status ?> block_<?= $robot_key + 1 ?>" type="button" <?if($allow_button):?>data-action="scan_<?= $temp_robot->robot_id.'_'.$temp_robot->robot_token ?>"<?endif;?>><?= $temp_robot->robot_name ?></a><?
        }
      }
      // If there were less than 8 robots, fill in the empty spaces
      if ($num_robots < 8){
...
if (window != window.top){
  ...
  <?
    ...
    /*
    // Create preload events for any of this player's active robots
    $robot_direction = $this_player->player_side == 'left' ? 'right' : 'left';
    foreach ($this_player->player_robots AS $this_key => $this_robotinfo){
      if (true || $this_robotinfo['robot_position'] == 'active'){
        echo "parent.mmrpg_preload_robot_sprites('{$this_robotinfo['robot_token']}', '{$robot_direction}');\r\n";
      }
    }
    // Create preload events for any of the target player's active robots
    $robot_direction = $target_player->player_side == 'left' ? 'right' : 'left';
    foreach ($target_player->player_robots AS $this_key => $this_robotinfo){
      if ($this_robotinfo['robot_position'] == 'active'){
        echo "parent.mmrpg_preload_robot_sprites('{$this_robotinfo['robot_token']}', '{$robot_direction}');\r\n";
      }
    }
    */
