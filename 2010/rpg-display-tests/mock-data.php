...
$robots['halloween_roll'] = generate_new_robot('Haloween Roll', 'halloween_roll', 'none', 'female', '2800', '15', '1320', $abilities[2]);
...
$turns[1] = generate_new_turn(1, 'hide', 'complete');
  ...
  $turns[1]['turn_events'][] = generate_new_turn_event($players[2], 'shout', time(), array('shout_text' => "I challenge you to a battle! With {$players[2]['player_robots'][0]['robot_name']} by my side, there&#39;s no way I can loose!"));
  $turns[1]['turn_events'][] = generate_new_turn_event($players[1], 'shout', time(), array('shout_text' => "I accept your challenge, but I <b>will</b> defeat you."));
$turns[2] = generate_new_turn(2, 'hide', 'complete');
  // Add some bs events to the turn for testing
  $turns[2]['turn_events'][] = generate_new_turn_event($players[1], 'shout', time(), array('shout_text' => "Nice moves, but it&#39;s gonna take more than nice moves to save you."));
$turns[3] = generate_new_turn(3, 'hide', 'complete');
  // Add some bs events to the turn for testing
  $turns[3]['turn_events'][] = generate_new_turn_event($players[2], 'shout', time(), array('shout_text' => "What the?!?!  Oh that&#39;s it - time to take out the big guns!"));
$turns[4] = generate_new_turn(4, 'hide', 'complete');
  // Add some bs events to the turn for testing
  $turns[4]['turn_events'][] = generate_new_turn_event($players[2], 'shout', time(), array('shout_text' => "Ahhhhhhhhhh! It can&#39;t end like this!"));
  $turns[4]['turn_events'][] = generate_new_turn_event($players[1], 'shout', time(), array('shout_text' => "Ha!  You call those big guns? Let {$players[1]['player_robots'][0]['robot_name']} show you what big guns really look like."));
$turns[5] = generate_new_turn(5, 'show', 'complete');
  // Add some bs events to the turn for testing
  $turns[5]['turn_events'][] = generate_new_turn_event($players[2], 'shout', time(), array('shout_text' => "OMG!  Nice work {$players[2]['player_robots'][0]['robot_name']}!"));
  $turns[5]['turn_events'][] = generate_new_turn_event($players[1], 'shout', time(), array('shout_text' => "Wha?!  How?!"));
  $turns[5]['turn_events'][] = generate_new_turn_event($players[1], 'shout', time(), array('shout_text' => "That&#39;s it!  It&#39;s time to get serious!"));
$turns[6] = generate_new_turn(6, 'show', 'current');
  // Add some events for the eighth turn
  $rand_winner = mt_rand(1, 2);
  $turns[6]['turn_events'][] = generate_new_turn_event($players[$rand_winner], 'shout', time(), array('shout_text' => "Yes!  We did it!!  Nice job {$players[$rand_winner]['player_robots'][0]['robot_name']} - there&#39;s no way I could have won without you. :)"));
  ...
  for ($i = 1; $i <= 5; $i++)
  {
    ...
    foreach ($turn_order AS $turninfo)
    {
      ...
      $turns[$i]['turn_events'][] = generate_new_turn_event($this_playerinfo, 'ability', time(), array('ability_user' => $this_robotinfo, 'ability_name' => $this_abilityinfo['ability_name'], 'ability_power' => $this_abilityinfo['ability_power'], 'ability_type' => $this_abilityinfo['ability_type'], 'ability_class' => $this_abilityinfo['ability_class']));
    }  	
  }
...
//    $this_robotinfo = $turninfo['robot_info'];
//    $this_abilityinfo = $this_robotinfo['robot_ability'];
//    $turns[8]['turn_events'][] = generate_new_turn_event($this_playerinfo, 'ability', time(), array('ability_user' => $this_robotinfo, 'ability_name' => $this_abilityinfo['ability_name'], 'ability_power' => $this_abilityinfo['ability_power'], 'ability_type' => $this_abilityinfo['ability_type'], 'ability_class' => $this_abilityinfo['ability_class']));
//  }
