...
<head>
<meta charset="UTF-8" />
<title>Mega Man RPG World | Battle Engine | Last Updated <?= preg_replace('#([0-9]{4})([0-9]{2})([0-9]{2})-([0-9]{2})#', '$1/$2/$3', MMRPG_CONFIG_CACHE_DATE) ?></title>
...
$event_timeout = !empty($_SESSION['RPG2k16']['GAME']['battle_settings']['eventTimeout']) ? $_SESSION['RPG2k16']['GAME']['battle_settings']['eventTimeout'] : 0;
...
<div id="battle" class="hidden">
  ...
  <form id="engine" action="scripts/battle.php<?= $flag_wap ? '?wap=true' : '' ?>" target="connect" method="post">
    ...
    <div class="wrapper">
      ...
      <div class="event sticky" style="z-index: 1;">
        ...
        if (!empty($this_field_data)){
          ...
          //echo '<div class="animate_fadein background_canvas background background_'.$background_animate[0].'" data-frame="'.$background_animate[0].'" '.(!empty($background_data_animate) ? 'data-animate="'.$background_data_animate.'"' : '').' style="background-image: url(images/sprites/fields/'.$this_field_data['field_background'].'/battle-field_background_base.gif?'.MMRPG_CONFIG_CACHE_DATE.');">&nbsp;</div>';
          echo '<div class="animate_fadein background_canvas background background_00" data-frame="00" style="background-image: url(images/sprites/fields/'.$this_field_data['field_background'].'/battle-field_background_base.gif?'.MMRPG_CONFIG_CACHE_DATE.');">&nbsp;</div>';
          ...
          if (!empty($this_field_data['field_background_attachments'])){
            ...
            foreach ($this_field_data['field_background_attachments'] AS $this_id => $this_info){
              ...
              echo '<div data-id="background_attachment_'.$this_key.'" class="sprite sprite_'.$this_boxsize.' sprite_'.$this_boxsize.'_'.$this_direction.' sprite_'.$this_boxsize.'_'.$this_frame.'" data-type="attachment" data-position="background" data-size="'.$this_size.'" data-direction="'.$this_direction.'" data-frame="'.$this_frame.'" data-animate="'.$this_animate.'" '.(!empty($this_animate_shift) ? 'data-animate-shift="'.$this_animate_shift.'"' : '').' style="'.$this_float.': '.$this_offset_x.'px; bottom: '.$this_offset_y.'px; z-index: '.$this_offset_z.'; background-image: url(images/sprites/'.$this_path.'/'.$this_token.'/sprite_'.$this_direction.'_'.$this_boxsize.'.png?'.MMRPG_CONFIG_CACHE_DATE.'); '.($debug_flag_spriteboxes ? 'background-color: rgba(255, 0, 0, 0.5); opacity: 0.75; ' : '').'">&nbsp;</div>';
            }
            echo '</div>';
          }
          ...
          echo '<div class="animate_fadein foreground_canvas foreground foreground_00" data-frame="00" style="background-image: url(images/sprites/fields/'.$this_field_data['field_foreground'].'/battle-field_foreground_base.png?'.MMRPG_CONFIG_CACHE_DATE.');">&nbsp;</div>';
          ...
          if (!empty($this_field_data['field_foreground_attachments'])){
            ...
            foreach ($this_field_data['field_foreground_attachments'] AS $this_id => $this_info){
              ...
              echo 'class="sprite sprite_'.$this_boxsize.' sprite_'.$this_boxsize.'_'.$this_direction.' sprite_'.$this_boxsize.'_'.$this_frame.'" data-type="attachment" data-position="foreground" data-size="'.$this_size.'" data-direction="'.$this_direction.'" data-frame="'.$this_frame.'" '.(!empty($this_animate_enabled) ? 'data-animate="'.$this_animate.'"' : '').' '.(!empty($this_animate_shift) ? 'data-animate-shift="'.$this_animate_shift.'"' : '').' style="'.$this_float.': '.$this_offset_x.'px; bottom: '.$this_offset_y.'px; z-index: '.$this_offset_z.'; background-image: url(images/sprites/'.$this_path.'/'.$this_image.'/sprite_'.$this_direction.'_'.$this_boxsize.'.png?'.MMRPG_CONFIG_CACHE_DATE.'); '.($debug_flag_spriteboxes ? 'background-color: rgba(255, 0, 0, 0.5); opacity: 0.75; ' : '').'">'.$this_text.'</div>';
            }
            echo '</div>';
          }
